...
| `candle-generated` (subscribed via `subscribeMessage::candle-generated`) | recv | `{"name":"candle-generated","microserviceName":"quotes","msg":{"active_id":86,"size":1,"at":<ns ts>,"from":<unix s>,"to":<unix s>,"id":<candle id>,"open":0.984425,"close":0.984425,"min":0.984425,"max":0.984425,"ask":0.98443,"bid":0.98442,"volume":0,"phase":"T"}}` | live price stream, very high frequency (~1 msg/sec at `size:1`, i.e. 1-second candles; 777 of these arrived during a ~5 minute session). **CORRECTED 2026-08-08, confirmed by a real live run at `size:60`**: push frequency (~1/sec) is INDEPENDENT of the subscribed `size` -- pushes represent live, continuously-updating state of the CURRENTLY-FORMING bar of that size (same `from`/`to` window repeated, with `close`/`min`/`max` evolving each push), not one push per closed bar. A new `from`/`to` window only starts once that bar's real duration has actually elapsed. `size:1` was the one case where update-cadence happens to equal bar-duration, which made this distinction invisible during the original spike (which only tested `size:1`) -- it was wrongly generalized to all `size` values, causing a real bug (premature strategy warm-up: a 20-candle warm-up completed in under a minute against `size:60` instead of the intended ~20 minutes). A consumer MUST detect window rollover (`from`/`open_time` changing between pushes) to know when a bar has actually closed; there is no explicit "closed" flag in the payload itself. See `src/botafuturo/adapters/exnova/market_data.py`'s module docstring for the fix (buffering the in-progress candle per window, yielding only on rollover). |
